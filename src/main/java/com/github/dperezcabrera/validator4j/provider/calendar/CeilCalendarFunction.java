 ...
 * @param <T>
 */
public class CeilCalendarFunction<T extends Calendar> extends IngoreNullFunction<T> {

    private int field;

    public CeilCalendarFunction(int field) {
        this.field = field;
    }

    @Override
    protected T applyNotNull(T item, Selector selector) {
        return (T) DateUtils.ceiling(item, field);
    }
}
