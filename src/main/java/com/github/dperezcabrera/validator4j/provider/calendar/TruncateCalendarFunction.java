 ...
 * @param <T>
 */
public class TruncateCalendarFunction<T extends Calendar> extends IngoreNullFunction<T> {

    private int field;

    public TruncateCalendarFunction(int field) {
        this.field = field;
    }

    @Override
    protected T applyNotNull(T item, Selector selector) {
        return (T) DateUtils.truncate(item, field);
    }
}
